...
<xmod:FormView runat="server"><AddItemTemplate><xmod:AddForm runat="server">
  <ScriptBlock BlockType="HeadScript" RegisterOnce="True" ScriptId="KBXM_Theme_ui-blitzer/jquery-ui-1.7.2.custom.css_Productor1">
    <link rel="stylesheet" type="text/css" href="/DNN/DesktopModules/XModPro/styles/ui-blitzer/jquery-ui-1.7.2.custom.css" />
  </ScriptBlock>

  <ScriptBlock BlockType="HeadScript" RegisterOnce="True" ScriptId="KBXM_Style_Productor1">
    <style type="text/css">
      .xmp-Productor1 { padding: 10px 5px 5px 5px; }
      .xmp-Productor1 .xmp-form-row { margin: 3px; padding: 3px; clear:left;}
      .xmp-Productor1 label.xmp-form-label, .xmp-Productor1 span.xmp-form-label{ display:block; float:left; width: 120px;text-align: left; margin-right: 5px; }
      .xmp-Productor1 .xmp-button { margin-right: 5px; }
    </style>
  </ScriptBlock>


  <SubmitCommand CommandText="INSERT INTO [XMP_Productor] ([localidad], [caracter], [id_provincia]) VALUES(@localidad, @caracter, @id_provincia) " /><ControlDataSource Id="id_provincia_tabla" CommandText="SELECT [id_provincia], [nombre_provincia], [id_pais] FROM [XMP_Provincias] ORDER BY [id_provincia] ASC"></ControlDataSource><ControlDataSource Id="id_pais" CommandText="SELECT [pais_id], [pais_nombre] FROM [XMP_Paises] ORDER BY [pais_id] ASC"></ControlDataSource><div class="ui-widget ui-widget-content xmp-form xmp-Productor1">
